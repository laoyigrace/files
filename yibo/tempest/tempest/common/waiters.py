...
def wait_for_vsan_cluster_status(client, cluster_id, status):
    """Waits for a Cluster to reach a given status."""
    body = client.show_vsan_cluster(cluster_id)
    cluster_status = body['status']
    start = int(time.time())

    while cluster_status != status:
        time.sleep(client.build_interval)
        body = client.show_vsan_cluster(cluster_id)
        cluster_status = body['status']
        if cluster_status == 'error':
            raise exceptions.ClusterBuildErrorException(cluster_id=cluster_id)
        if cluster_status == 'error_replace_disk':
            raise exceptions.ClusterReplaceErrorException(cluster_id=cluster_id)
        if cluster_status == 'error_expand':
            raise exceptions.ClusterExpandErrorException(cluster_id=cluster_id)

        if int(time.time()) - start >= client.build_timeout:
            message = ('Cluster %s failed to reach %s status (current %s) '
                       'within the required time (%s s).' %
                       (cluster_id, status, cluster_status,
                        client.build_timeout))
            raise exceptions.TimeoutException(message)

def wait_for_vsan_resource_deletion(client, id):
    ...
    start_time = int(time.time())
    while True:
        if client.is_resource_deleted(id):
            return
        if int(time.time()) - start_time >= client.delete_timeout:
            message = ('Failed to delete %(resource_type)s %(id)s within '
                       'the required time (%(timeout)s s).' %
                       {'resource_type': client.resource_type, 'id': id,
                        'timeout': client.delete_timeout})
            caller = misc_utils.find_test_caller()
            if caller:
                message = '(%s) %s' % (caller, message)
            raise exceptions.TimeoutException(message)
        time.sleep(client.delete_interval)
