...
class IsolatedCreds(cred_provider.CredentialProvider):
    ...
    def _create_creds(self, suffix="", admin=False, roles=None):
        ...
        LOG.info("+++sf, username = %s, user_password = %s",
        ...
        for cred in self.isolated_creds:
            ...
            if (not self.network_resources or
                ...
                self._clear_isolated_router(creds.router['id'],
                                            creds.router['name'])
            # wait remove route success
            i = 0
            while i < 60:
                if not creds.router:
                    break
                router_list = self.network_admin_client.list_routers()['routers']
                if creds.router['id'] not in [n['id'] for n in router_list]:
                    break
                time.sleep(1)
